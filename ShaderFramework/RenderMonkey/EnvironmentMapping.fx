...
VS_OUTPUT EnvironmentMapping_Pass_0_Vertex_Shader_vs_main(VS_INPUT Input)
{
   ...
   float3 worldTanget = mul(Input.mTangent, (float3x3)gWorldMatrix);
   Output.T = normalize(worldTanget);
...
{
   // Noraml form NormalMap in TangentSpace 
   float3 tangetNormal = tex2D(NormalSampler, Input.mUV);
   tangetNormal = normalize(tangetNormal * 2 - 1);
   tangetNormal = float3(0,0,1);
   
   // Normal in WorldSpace
   float3x3 TBN = float3x3(normalize(Input.T), normalize(Input.B), normalize(Input.N));
   TBN = transpose(TBN);
   float3 worldNormal = mul(TBN, tangetNormal);
